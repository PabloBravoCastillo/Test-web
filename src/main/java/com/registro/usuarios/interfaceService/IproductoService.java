...
public interface IproductoService extends JpaRepository<Producto, Integer> {
    ...
    @Query(value="UPDATE `producto` SET `nombre` = :nombre, `descripcion` = :descripcion, `precio` = :precio, `stock` = :stock "
    		+ ", `categoria_id_categoria` = :categoria_id_categoria, `plataforma_id_plataforma` = :plataforma_id_plataforma "
    		+ "WHERE `producto`.`id_producto` = :id", nativeQuery=true)
    void actualizarProducto(@Param("id") int id_producto, @Param("nombre") String nombre, @Param("descripcion") String descripcion, 
    		@Param("precio") double precio, @Param("stock") int stock
    		, @Param("categoria_id_categoria") int categoria_id_categoria, @Param("plataforma_id_plataforma") int plataforma_id_plataforma );
}
